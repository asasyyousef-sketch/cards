          ...
          <div className="w-full aspect-square rounded-lg overflow-hidden relative mb-4 border border-outline-variant/10 flex items-center justify-center bg-surface-container-low select-none">
            ...
            <div className="absolute top-3 left-3 flex items-center gap-1.5 z-30">
              <button
                ...
                className="p-2.5 rounded-lg bg-white/90 text-primary hover:bg-primary hover:text-white transition-all shadow-sm cursor-pointer border border-slate-100/50"
                title="استمع للنطق الأساسي"
              >
              ...
              {onPlaySecondaryPronunciation && (
                <button
                  ...
                  className="p-2.5 rounded-lg bg-indigo-600/90 text-white hover:bg-indigo-700 transition-all shadow-sm cursor-pointer border border-indigo-500/50 flex items-center justify-center relative group"
                  title="استمع للنطق الثانوي (الصوت المفعل)"
                >
                  <Volume2 className="w-4.5 h-4.5" />
                </button>
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              {onPlaySecondaryPronunciation && (
                <button
                  ...
                  className="w-9 h-9 rounded-lg bg-indigo-500/10 text-indigo-600 hover:bg-indigo-500/20 transition-all cursor-pointer border border-indigo-500/20 flex items-center justify-center relative"
                  title="نطق النص الحالي (الثانوي)"
                >
                  <Volume2 className="w-4.5 h-4.5" />
                </button>
              )}
...
export const ReviewSession: React.FC<ReviewSessionProps> = React.memo(({
  ...
  const playPronunciation = (text: string, lang: string, bypassMute = false, voiceOverride?: string) => {
    ...
    if (text && text.trim() && currentCard) {
      lastSpokenKeyRef.current = `${currentCard.id}_${text.trim()}`;
    }
    stopCurrentAudio();
    speakClient(text, lang, voiceOverride);
  };

  const playSecondaryPronunciation = (text: string, lang: string) => {
    const langShort = (lang || "de").toLowerCase().split("-")[0].split("_")[0];
    const secondaryVoice = localStorage.getItem(`settings_secondary_piper_model_${langShort}`) || 
                           localStorage.getItem("settings_secondary_piper_model") || 
                           "google";
    playPronunciation(text, lang, true, secondaryVoice);
  };
