...
4. checklist: مصفوفة متسلسلة تضمن التغطية الشاملة والكاملة (100% Coverage) لكل المتغيرات والشروط المدخلة في سياق التمرين.
  ...
  app.post("/api/ai/review-chat", express.json(), async (req, res) => {
    try {
      ...
      let lengthDirective = "متوسط الطول ومركز ومباشر، يركز على الفائدة اللغوية وتنسيق القواعد بوضوح دون إطالة غير ضرورية.";
      if (responseLength === "concise") {
        lengthDirective = "مختصر جداً ومركز وسريع القراءة (فقرة أو نقاط سريعة).";
      } else if (responseLength === "detailed") {
        lengthDirective = "شامل ومفصل مع أمثلة حية وشرح لقواعد النحو وتصريفات الأفعال وجداول واستخدامات متعددة.";
      }

      // Format previous and next cards context
      const prevContext = Array.isArray(previousCards) && previousCards.length > 0
        ? previousCards.map((c: any, i: number) => `  ${i + 1}. [${c.correctArticle ? c.correctArticle + ' ' : ''}${c.frontText}] -> (${c.backText})${c.translationHint ? ` [تلميح: ${c.translationHint}]` : ''}`).join('\n')
        : "  (لا توجد بطاقات سابقة)";

      const nextContext = Array.isArray(nextCards) && nextCards.length > 0
        ? nextCards.map((c: any, i: number) => `  ${i + 1}. [${c.correctArticle ? c.correctArticle + ' ' : ''}${c.frontText}] -> (${c.backText})${c.translationHint ? ` [تلميح: ${c.translationHint}]` : ''}`).join('\n')
        : "  (لا توجد بطاقات تالية)";

      const wantsImages = Boolean(req.body.includeImages) || 
        /صور|صورة|صورا|photos?|images?|visual|illustrations?|مرئي|بصري|مكونات|أشكال/i.test(message);

      let imageInstruction = "";
      if (wantsImages) {
        imageInstruction = `
6. 🖼️ **إدراج الصور التوضيحية البصرية (Visual Images Integration)**:
   - الطالب يرغب في دعم بصري ورؤية صور توضيحية للمفاهيم والمفردات والمكونات والسياقات.
   - لطلب صورة توضيحية في مكان مناسب من الشرح، أدرج الوسم التالي:
     \`$$IMAGE:{"query":"Speisekarte restaurant menu","caption":"قائمة طعام (Speisekarte)","size":"large","keyword":"die Speisekarte"}$$\`
   - أو لمجموعة صور مصغرة (مثل المكونات أو المرادفات أو الأجزاء أو المقارنة بين الأشياء):
     \`$$IMAGES:[{"query":"Apfel fruit","caption":"تفاح طازج","size":"small","keyword":"der Apfel"},{"query":"Apfelkuchen slice","caption":"كعكة التفاح","size":"small","keyword":"der Apfelkuchen"}]$$\`
   - **قواعد الأحجام (size)**:
     - \`"large"\`: صورة رئيسية بارزة وواضحة للمفهوم الأساسي أو الكلمة الرئيسية مع الشرح والعنوان.
     - \`"medium"\`: صورة متوسطة مناسبة لمثال أو سياق استخدام.
     - \`"small"\`: صور مصغرة مدمجة (ممتازة للأجزاء والمكونات والمفردات المتعددة).
   - **حقل \`query\`**: اكتب كلمات بحثية واضحة ودقيقة بالإنجليزية أو الألمانية (مثل "restaurant menu card", "fresh red apple") تضمن لمحرك البحث جلب صور حقيقية عالية الجودة.
   - **حقل \`keyword\`**: الكلمة الأجنبية المستهدفة المرتبطة بالصورة ليتمكن المستخدم من الاستماع لنطقها أو نسخها.
   - **حقل \`caption\`**: شرح عربي موجز ومفيد يصف ما تعبر عنه الصورة.`;
      }
...
أنت متواجد حالياً مع الطالب أثناء مراجعته للبطاقات التعليمية، ومهمتك تقديم شرح ذكي، تصحيح دقيق لجمل المستخدم، توضيح الفروقات الدلالية والقواعد وأدوات التعريف، وضرب أمثلة سياقية حية.
...
🃏 **البطاقة الحالية قيد المراجعة والمناقشة**:
...
⏮️ **البطاقات السابقة (الـ 5 السابقة في الجلسة)**:
${prevContext}

⏭️ **البطاقات التالية (الـ 5 التالية في الجلسة)**:
${nextContext}

✨ **تعليمات أسلوب الرد والتنسيق المعتمدة (مطابقة لقسم صحح)**:
1. **طول الإجابة وتنسيقها**: ${lengthDirective}
2. **التنسيق الهيكلي (Structured Markdown)**:
   - استخدم العناوين الفرعية المنظمة بالأرقام مثل: \`### 1. **أداة التعريف (Der Artikel) - \\\`die\\\`**\` أو \`### 2. **صيغة الجمع (Der Plural)**\`
   ...
   - استخدم جداول Markdown (\`| العمود 1 | العمود 2 |\`) لتنظيم تصريفات الأفعال، أدوات التعريف، أو مقارنة المرادفات لتسهيل قراءتها.
